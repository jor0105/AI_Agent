...
class _TabularDataFrame(Protocol):
    """Subset of a pandas DataFrame used by the file readers."""

    empty: bool
    shape: tuple[int, ...]

    def to_string(self, *, index: bool) -> str:
        """Render the tabular content as text."""


class _PandasReader(Protocol):
    """Pandas operations required by this optional integration."""

    def read_csv(self, file_path: Path, **kwargs: Any) -> _TabularDataFrame:
        """Load a CSV file."""

    def read_excel(self, file_path: Path, **kwargs: Any) -> _TabularDataFrame:
...
def _require_pandas(purpose: str) -> _PandasReader:
    ...
    return cast(_PandasReader, pandas)
